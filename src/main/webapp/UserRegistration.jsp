...
<div style="max-width:390px;padding:5px;margin:auto;">
    ...
    <form class="form-horizontal" id="formRegister" data-toggle="validator" role="form">
        <div class="form-group">
            ...
            <input name="<%=FieldConstants.INPUT_EMAIL%>" pattern="<%=RegexConstants.REGEX_VALID_EMAIL%>"
                   class="form-control"
                   id="<%=FieldConstants.INPUT_EMAIL%>" placeholder="Enter Email"
                   data-error="<%=ErrorMessageConstants.ERROR_MSG_INVALID_EMAIL%>" required>
            <div class="help-block with-errors"></div>
        </div>
        <div class="form-group">
            <label for="<%=FieldConstants.INPUT_FIRSTNAME%>" class="control-label">First Name</label>
            <input pattern="<%=RegexConstants.REGEX_VALID_ALPHA_NUMERIC_STRING%>{1,20}"
                   name="<%=FieldConstants.INPUT_FIRSTNAME%>" class="form-control"
                   id="<%=FieldConstants.INPUT_FIRSTNAME%>" placeholder="Enter First Name"
                   data-error="<%=ErrorMessageConstants.ERROR_MSG_INVALID_NAME%>" required>
            <div class="help-block with-errors"></div>
        </div>
        <div class="form-group">
            <label for="<%=FieldConstants.INPUT_LASTNAME%>" class="control-label">Last Name</label>
            <input pattern="<%=RegexConstants.REGEX_VALID_ALPHA_NUMERIC_STRING%>{1,20}"
                   name="<%=FieldConstants.INPUT_LASTNAME%>" class="form-control"
                   id="<%=FieldConstants.INPUT_LASTNAME%>" placeholder="Enter Last Name"
                   data-error="<%=ErrorMessageConstants.ERROR_MSG_INVALID_NAME%>" data-toggle="tooltip"
                   data-placement="right" required>
            <div class="help-block with-errors"></div>
        </div>
        <div class="form-group">
            <label for="<%=FieldConstants.INPUT_PASSWORD%>" class="control-label">Password</label>
            <input type="password" pattern="<%=RegexConstants.REGEX_VALID_PASSWORD%>"
                   name="<%=FieldConstants.INPUT_PASSWORD%>" class="form-control"
                   id="<%=FieldConstants.INPUT_PASSWORD%>" placeholder="Enter Password"
                   data-error="<%=ErrorMessageConstants.ERROR_MSG_INVALID_PASSWORD%>" required>
            <div class="help-block with-errors"></div>
        </div>
        <div class="form-group">
            ...
            <input type="password" name="<%=FieldConstants.INPUT_PASSWORD_CONFIRM%>" class="form-control"
                   id="<%=FieldConstants.INPUT_PASSWORD_CONFIRM%>" data-match="#inputPassword"
    ...
    $(function () {
        $('#formRegister').validator().on('submit', function (e) {
            if (e.isDefaultPrevented()) {
                ...
                $.post("UserRegistration", $("#formRegister").serialize(), function (data) {
                    console.log(data);
                    var jdata = JSON.parse(data);
                    console.log(jdata.code === -1);
                    if (jdata.code === -1) {
